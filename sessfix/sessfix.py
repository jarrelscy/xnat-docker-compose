...
def relabel_session_files(session, oldlabel, olddir, thinslice_mod=None):
    ...
    if not op.isdir(olddir):
        ...
        if op.isfile(oldxml):
            with open(oldxml, 'r') as f:
                accession, subject = f.read().strip().replace('REMOVED NEW ACCESSION ', '').split('#')
                ...
                return accession, newdir, subject
        else:
    ...
    if source_ae:
        if 'source_ae' in dicom_utils.ae_titles:
            thinslice_mod = dicom_utils.ae_titles[source_ae][2]
        else:
            thinslice_mod = 'ACC' # default to thinslice_mod if we don't know where it comes from 
    else:
    ...
    subject = sessnode.xpath('//xnat:dcmPatientId', namespaces=sessnode.nsmap)[0].text
    try:
        ...
        with open(oldxml, 'w') as f:
            f.write('REMOVED NEW ACCESSION {}#{}'.format(accession, subject))
        shutil.rmtree(olddir)
        
        if source_ae in SOURCE_AE_MAP:
            if session.query(Request).filter(Request.new_accession == accession).filter(Request.new_patient_id == subject).filter(Request.application_entity == source_ae).filter(Request.status != 'COMPLETED').scalar() is None:
                session.add(Request(patient_id='autoupload', 
                accession='autoupload',
                request_id='autoupload-'+str(uuid.uuid1()).replace('-', ''), 
                override=False,
                last_updated=datetime.datetime.now(),
                created=datetime.datetime.now(), 
                new_accession=accession, 
                new_patient_id=subject, 
                application_entity=source_ae, 
                status="DICOMSENT", # bypass check for DICOMSENT
                project=SOURCE_AE_MAP[source_ae]))
...
def main(session):
    ...
    updated_names = []
    pending_alfred_ids_query = session.query(Request.patient_id).filter(Request.status != 'COMPLETED').filter(Request.status != 'FAILED').subquery()
    prearchive_experiments = session.query(PrearchiveExperiment).filter(PrearchiveExperiment.status == 'READY').filter(or_(PrearchiveExperiment.name.in_(pending_alfred_ids_query), PrearchiveExperiment.project == 'ALFRED_TEMP'))
    prearchive_names = []
    for row in prearchive_experiments:
        ...
        try:
            new_name, newdir, new_subject = relabel_session_files(session, old_name, sessdir)
            logger.info ('Relabelled oldname: {} sessdir: {} newname: {} newdir: {} subject {}'.format(old_name, sessdir, new_name, newdir, new_subject))
            if new_name and newdir:                
                
                updated_names.append(new_name)
                row.subject = new_subject # modify subject in prearchive table as xnat no longer seems to do this with the anonymization script
...
def getHash(accession, ae):    
    return (dicom_utils.ae_titles[ae][2] if ae else '') + requests.get('http://{}:8888/'.format(HASHERURL)+accession).text
    ...
    for s in sess.prearchive.sessions():
        try:
            ...
            if s.data['status'] == 'READY' and s.data['name'][:2] == 'AC':
                ...
                #s.rebuild(asynchronous=True)
                logger.info (s.data['status'])
        except IndexError as e:
            logger.info("Index error, likely at s.data['status'] == 'RECEIVING' in do_xnat_stuff")
        except Exception as e:
            logger.error(traceback.format_exc())
    sess.clearcache()
    for s in list(sess.prearchive.sessions()):
        if (s.data['status'] == 'READY' or s.data['status'] == 'ERROR') and s.data['name'][:2] == 'AC':
            s.rebuild(asynchronous=False)
    sess.clearcache()
    time.sleep(3)
    for s in sess.prearchive.sessions():
        try:
...
if __name__ == '__main__':   
    ...
    while True:
        try:
            try:
                with open('/data/xnat/source_ae_map.json', 'r') as f:
                    SOURCE_AE_MAP = json.load(f)
            except:
                logger.warning ('Could not load source ae map from /data/xnat/source_ae_map.json {}', traceback.format_exc())
            ...
            with session_scope(Session) as session:
                ...
                updated_names, prearchive_names = main(session)
                ...
                # POSSIBLE request states - PENDING, DICOMSENDING, DICOMSENT, PENDINGSHARE, COMPLETED
                ...
                for i in session.query(Request).filter(Request.status == 'PENDINGSHARE'):
                    try:
                        if i.new_accession is None:
                            hash = str(getHash(i.accession, i.application_entity)) 
                        else:
                            hash = i.new_accession
                        if i.new_patient_id is None:
                            pthash = str(getHash(i.patient_id, ''))
                        else:
                            pthash = i.new_patient_id
                ...
                if hour >= 23 or hour < 8:                    
                    requests_pending = [i for i in session.query(Request).filter(Request.status == 'PENDING').filter(Request.override.is_(False)).order_by(Request.last_updated)]
                    requests_sorted_by_id = [[i for i in requests_pending if i.request_id == r] for r in request_ids]
                    not_done = True
                    while not_done:
                        not_done = False
                ...
                just_added = []                 
                for i in request_interleaved[:MAX_NUMBER_RECEIVING]:
                    ...
                    if found_in_alfred:
                        ...
                        #logger.info ('Num receiving {} just_added {} MAX_NUMBER_RECEIVING {} input_queue {} NUM_POOL_WORKERS {}'.format(num_receiving, len(just_added), MAX_NUMBER_RECEIVING, input_queue.qsize(), NUM_POOL_WORKERS))
                        if (num_receiving+len(just_added)) < MAX_NUMBER_RECEIVING and input_queue.qsize() < NUM_POOL_WORKERS:
                            ...
                            logger.info ('Finished submit job to DICOM queue {} {} {} {}'.format(i.accession,i.patient_id, i.application_entity, i.request_id))
                        else:
                            pass                    
                session.commit()
